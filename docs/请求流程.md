...
4. **Model / Data / Tools**  
   ...
   - Data：短时是 `messages`；长期是 `memories`。`prompt_block()` 读后者。  
   - Tools：在本机跑。`remember` 写 Postgres，不是写进模型权重。

5. **收尾**  
   图跑完（或点终止）后，Gateway 把拼好的回答写入 `messages`，再发 `done`。下次同一会话，这轮问答出现在短时窗口里；长期记忆不依赖会话，新对话也会被注入。

侧栏会话列表来自 `messages`；侧栏长期记忆来自 `memories`。删会话不会清长期记忆。
